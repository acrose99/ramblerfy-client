...
def main(sp, search_string, search_type):
    ...
    # returns top 5 tracks for given artist (top results generated by spotify)
    if str(search_type).upper() == "ARTIST":
        data = sp.search(q='artist:' + '"' + search_string + '"', limit=5, offset=0, type='track', market='US')
        tracks = data["tracks"]["items"]
        pprint.pprint("Searching for " + search_string + "...")
...
def search_multiples(sp, searches, search_type):
    for search in searches:
        main(sp, search, search_type)
    audio_features.write_json()
